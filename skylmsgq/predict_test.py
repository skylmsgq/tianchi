...
def func6():
	...
	for date in train:
		...
		fileinput.close()
		for line in gzip.open("feature/user/"+date+".txt.gz"):
			umap[line.strip().split("\t")[0]] = [float(i) for i in line.strip().split("\t")[1:]]
		fileinput.close()
	...
	for line in gzip.open("feature/iid/"+predict+".txt.gz"):
		imap[line.strip().split(" ")[0]] = [float(i) for i in line.strip().split(" ")[1:]]
	fileinput.close()
	for line in gzip.open("feature/user/"+predict+".txt.gz"):
		umap[line.strip().split("\t")[0]] = [float(i) for i in line.strip().split("\t")[1:]]
	fileinput.close()
	file = open("predict_test.txt","w")
	num, tot, cor = 0, 0, 0
